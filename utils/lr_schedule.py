...
def cosine_decay(args, batchs: int, decay_type: int = 1):
    ...
    if decay_type == 1:
        schedule = np.array([1e-12 + 0.5 * (args.max_lr - 1e-12) * (1 + math.cos(math.pi * t / total_batchs)) for t in iters])
    elif decay_type == 2:
    ...
    # np.clip(schedule, args.min_lr, args.max_lr, out=schedule)

    if args.warmup_batchs > 0:
        # warmup_lr_schedule = np.linspace(args.warmup_lr, args.max_lr, args.warmup_batchs)
